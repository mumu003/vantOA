 ...
 <van-collapse-item
        ...
        >
          <van-cell-group v-if="showlist.length>0">
            <van-cell
              :title="'姓名:'+citem.name"
              :value="'电话:'+citem.phone"
              v-for="(citem,cindex) in showlist"
              :key="citem.id+cindex"
            />
          </van-cell-group>
          <!-- 如果没数据 -->
...
<script>
import { findAlldepart, findAllList, adddepart, updatedepart, deletedepart } from "@/api/depart";
import manageIntegral from '@/components/manageintegral.vue'
export default {
  components:{
    manageIntegral
  },
